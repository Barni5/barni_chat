...
AddEventHandler('chatMessage', function(source, name, message)
  if string.sub(message, 1, string.len('/')) ~= '/' then CancelEvent()
    ...
    local admin = false
    if barni.LOOCAdminPrefixes then
      if barni.AdminGroups[xPlayer.getGroup()] then admin = true end
    end
    TriggerClientEvent('barni_rpchat:sendLocalOOC', -1, source, name, message, admin);
  end

end)


RegisterCommand(barni.Commands['Staff Announcement'], function(source,args,raw)
  local xPlayer = ESX.GetPlayerFromId(source)
  local toSay = ''
    for i=1,#args do
     toSay = toSay .. args[i] .. ' ' 
   ...
   if barni.AdminGroups[xPlayer.getGroup()] == true then
    TriggerClientEvent('chat:addMessage', -1, {
      template = [[
        <div style="padding: 0.3vw; height: fit-content; margin: 0.5vw; margin: 0.5vw; background-color: #222228ec; border-radius: 10px; padding-top: 0.35vw; border: 0.11vw solid #414146; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.35)">
...
RegisterCommand('id', function(source,args,raw)
    TriggerClientEvent('chat:addMessage', source, {
      template = [[
        <div style="padding: 0.3vw; height: fit-content; margin: 0.5vw;  background-color: #222228ec; border-radius: 10px; padding-top: 0.35vw; border: 0.11vw solid #414146; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.35)">
...
RegisterCommand(barni.Commands['Job'], function(source,args,raw)
...
if barni.AutoMessages then
  Citizen.CreateThread(function()
    ...
    TriggerClientEvent('chat:addMessage', -1, {
      ...
      ]],      args = { barni.AutoMessagesList[math.random(#barni.AutoMessagesList)] }
    })
    Citizen.Wait(barni.AutoMessageInterval*60*1000)
  end
  end)
end
RegisterCommand(barni.Commands['Blackmarket'], function(source, args, raw)
  local xPlayer = ESX.GetPlayerFromId(source)
  local toSay = ''
    for i=1,#args do
  toSay = toSay .. args[i] .. ' '
  end
  if xPlayer.job.name ~= barni.Jobs['LSPD'] then
    TriggerClientEvent('chat:addMessage', -1, {
      template = [[
        <div style="padding: 0.3vw; height: fit-content; margin: 0.5vw;  background-color: #222228ec; border-radius: 10px; padding-top: 0.35vw; border: 0.11vw solid #414146; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.35)">
        <font style="padding: 0.15vw; margin: 0.22vw; background-color: #626066; padding-left: 1%; border-radius: 8px; border: #9e9e9e solid 0.11vw; font-size: 12px; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.55)">
...
local adcooldown = {}
RegisterCommand(barni.Commands['Advertisement'], function(source, args, raw)
  local xPlayer = ESX.GetPlayerFromId(source)
  local toSay = ''
    for i=1,#args do
  toSay = toSay .. args[i] .. ' '
  end
  if xPlayer.getAccount('bank').money >= barni.AdCost then
    if not adcooldown[source] then
      ...
      xPlayer.removeAccountMoney('bank',barni.AdCost)
      adcooldown[source] = true
      Citizen.SetTimeout(barni.AdCooldown,function()
...
RegisterCommand(barni.Commands['Twitter'], function(source, args, raw)
  local fal = GetRealPlayerName(source)
  local toSay = ''
  for i=1,#args do
    toSay = toSay .. args[i] .. ' '
  ...
  TriggerClientEvent('chat:addMessage', -1, {
    template = [[
      ...
      <font style="font-size: 14px; padding-left: 5px; color: #c7c5c5; margin: auto; font-weight:100">@{1}:</font>
      <font class="message" style="font-size: 14px; color: #a8a7a7; margin: auto; font-weight:100"> {2}</font>
  </div>
    ]],      args = {source,fal, toSay}
  })
end, false)

RegisterCommand(barni.Commands['Police'], function(source, args, raw)
...
RegisterCommand(barni.Commands['Sheriff'], function(source, args, raw)
  local playerName = GetPlayerName(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  local command = "ambulance"
  local toSay = ''
  ...
  if xPlayer.job.name == barni.Jobs['LSSD'] then 
  TriggerClientEvent('chat:addMessage', -1, {
    template = [[
      <div style="padding: 0.3vw; height: fit-content; margin: 0.5vw;  background-color: #222228ec; border-radius: 10px; padding-top: 0.35vw; border: 0.11vw solid #414146; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.35)">
      <font style="padding: 0.15vw; margin: 0.22vw; background-color: #626066; padding-left: 1%; border-radius: 8px; border: #9e9e9e solid 0.11vw; font-size: 12px; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.55)">
...
RegisterCommand(barni.Commands['EMS'], function(source, args, raw)
  local playerName = GetPlayerName(source)
  local xPlayer = ESX.GetPlayerFromId(source)
  local command = "ambulance"
  local toSay = ''
  ...
  if xPlayer.job.name == barni.Jobs['EMS'] then 
  TriggerClientEvent('chat:addMessage', -1, {
    template = [[
      <div style="padding: 0.3vw; height: fit-content; margin: 0.5vw;  background-color: #222228ec; border-radius: 10px; padding-top: 0.35vw; border: 0.11vw solid #414146; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.35)">
      <font style="padding: 0.15vw; margin: 0.22vw; background-color: #626066; padding-left: 1%; border-radius: 8px; border: #9e9e9e solid 0.11vw; font-size: 12px; box-shadow: 0px 0px 11px rgba(158, 158, 158, 0.55)">
...
RegisterCommand('me', function(source, args, raw)
  ...
  TriggerClientEvent('barni_rpchat:sendMe', -1, source, name, args, { 196, 33, 246 })
...
RegisterCommand('do', function(source, args, raw)
  ...
  TriggerClientEvent('barni_rpchat:sendDo', -1, source, name, args, { 255, 198, 0 })
...
RegisterCommand('try', function(source, args, raw)
  ...
  if random == 1 then
    result = "YES"
    TriggerClientEvent('barni_rpchat:sendTry', -1, source, name, result, { 255, 198, 0 })
...
RegisterCommand('doc', function(source, args, raw)
  ...
  if args[1] ~= nil then 
    ...
    if count < barni.MaxDocCount then
      while c < count do
          c = c + 1 
          TriggerClientEvent('barni_rpchat:sendDoc', -1, source, name, c .. "/" .. count , { 255, 198, 0 })
