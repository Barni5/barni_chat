...
AddEventHandler('playerDropped', function (reason)
    ...
    if playerStatus[source] then
        TriggerClientEvent('barni_rpchat:RemovePlayerStatus', -1, source)
    end
end)

RegisterCommand(barni.Commands['Status'], function(source, args, rawCommand)
    ...
    if xPlayer then
        if playerStatus[_source] then
            ...
            TriggerClientEvent('barni_rpchat:RemovePlayerStatus', -1, _source)
            Notify('Success',"You have removed your status")
        else
            local message = table.concat(args, ' ', 1)
            playerStatus[_source] = message
            TriggerClientEvent('barni_rpchat:SetPlayerStatus', -1, _source, message)
...
RegisterNetEvent('barni_rpchat:RequestMessages')
AddEventHandler('barni_rpchat:RequestMessages', function()
    TriggerClientEvent('barni_rpchat:SetMessages', source, displayedMessages, playerStatus)
end)
