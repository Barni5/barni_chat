...
local function havePermission(xPlayer)
	...
	if barni.AdminGroups[group] then
		return true
	end
	return false
end

ESX.RegisterServerCallback('barni_rpchat:getPlayerGroup', function(source, cb)
...
RegisterCommand("report", function(source, args, raw)
	...
	TriggerClientEvent("barni_rpchat:send", -1, source, name, content)
