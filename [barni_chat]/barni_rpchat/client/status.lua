...
RegisterCommand(barni.Commands['Here'], function(source, args, rawCommand)
    if playerLoaded then
        ...
        if not displayedMessages[roundedCoords] then
            ...
            if playerMessages > barni.MaxHereTexts then
                Notify('Error',"You can't place more than "..barni.MaxHereTexts..' texts')
            else
                local msg = ''
                for i = 1,#args do
                    msg = msg .. ' ' .. args[i]
...
RegisterNetEvent('barni_rpchat:SetMessages')
AddEventHandler('barni_rpchat:SetMessages', function(heres, statuses)
...
RegisterNetEvent('barni_rpchat:SetPlayerStatus')
AddEventHandler('barni_rpchat:SetPlayerStatus', function(playerId, message)

    playerStatuses[playerId] = message

end)

RegisterNetEvent('barni_rpchat:RemovePlayerStatus')
AddEventHandler('barni_rpchat:RemovePlayerStatus', function(playerId)
...
Citizen.CreateThread(function()
    TriggerServerEvent('barni_rpchat:RequestMessages')
    while true do
        Wait(0)
        local letSleep = true
        local pedCoords = GetEntityCoords(PlayerPedId())
...
function DrawText3DsH(x,y,z, text)
    ...
    if not barni.TransparentStatusHere then
...
function DrawText3DsS(x,y,z, text)
    ...
    DrawText(_x, _y)
    local factor = (string.len(text)) / 320
    if barni.TransparentStatusHere then
