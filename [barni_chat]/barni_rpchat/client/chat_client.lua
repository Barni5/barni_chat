...
RegisterNetEvent('barni_rpchat:sendMe')
AddEventHandler('barni_rpchat:sendMe', function(playerId, title, message, color)
...
RegisterNetEvent('barni_rpchat:sendTry')
AddEventHandler('barni_rpchat:sendTry', function(playerId, title, message, color)
    local source = GetPlayerServerId(PlayerId())
    local target = GetPlayerFromServerId(playerId)
    local sourcePed, targetPed = PlayerPedId(), GetPlayerPed(target)
    local sourceCoords, targetCoords = GetEntityCoords(sourcePed), GetEntityCoords(targetPed)
...
RegisterNetEvent('barni_rpchat:sendDo')
AddEventHandler('barni_rpchat:sendDo', function(playerId, title, message, color)
  local source = GetPlayerServerId(PlayerId())
  local target = GetPlayerFromServerId(playerId)
  local sourcePed, targetPed = PlayerPedId(), GetPlayerPed(target)
  local sourceCoords, targetCoords = GetEntityCoords(sourcePed), GetEntityCoords(targetPed)
...
RegisterNetEvent('barni_rpchat:sendDoc')
AddEventHandler('barni_rpchat:sendDoc', function(playerId, title, message, color)
    local source = GetPlayerServerId(PlayerId())
    local target = GetPlayerFromServerId(playerId)
    local sourcePed, targetPed = PlayerPedId(), GetPlayerPed(target)
    local sourceCoords, targetCoords = GetEntityCoords(sourcePed), GetEntityCoords(targetPed)
...
RegisterNetEvent('barni_rpchat:sendLocalOOC')
AddEventHandler('barni_rpchat:sendLocalOOC', function(playerId, title, message, admin)
	local source = GetPlayerServerId(PlayerId())
	local target = GetPlayerFromServerId(playerId)
...
RegisterNetEvent('barni_rpchat:getCoords')
AddEventHandler('barni_rpchat:getCoords', function(player)
	  local ped = PlayerPedId()
    local coords = GetEntityCoords(ped, false)
    local heading = GetEntityHeading(ped)

    local message = tostring("X: " .. coords.x .. " Y: " .. coords.y .. " Z: " .. coords.z .. " HEADING: " .. heading)
    TriggerServerEvent('barni_rpchat:showCoord', player, message)
